...
def main():
    ...
    # Paired mode - not storing any location values
    min_loc = -1
    timer = 0
    for seed_range in seeds_paired:
        for i in range(seed_range[0], seed_range[1]):
            ...
            for mapping in mappings:
                ...
                if (stage == 6 and (min_loc == -1 or src < min_loc)): min_loc = src
                stage += 1
            percent = (timer / 11687205618 * 100)
            print(str(percent) + " - processed " + str(timer))
            timer += 1
